...
%
% Calculate NC-HIPP coherence time-locked to ripples and NREM-control
% events
%
% Requirements:
...
svName  = 'coherence';                                  %% filename for saved data
...
out.dof         = [];
out.coh.ripple  = nan(numFreq,numTime);
out.coh.ctrl    = nan(numRep,numFreq,numTime);
...
fprintf('... step 2b: calculate coherence for ripples\n');
...
tmp_coh          = ft_connectivityanalysis(tfg,tmpData);

out.coh.ripple(:,:) = squeeze(tmp_coh.cohspctrm(1,2,:,:));
...
for iRep = 1 : numRep
    ...
    %% calculate coherence
    fprintf('... step 3b (rep %d): calculate coherence for control events\n', iRep);
    ...
    out.coh.surrgt(iRep,:,:) = squeeze(tmp_coh.cohspctrm(1,2,:,:));
...
tmpRipple       = out.coh.ripple;
tmpCtrl_mean    = squeeze(nanmean(out.coh.ctrl,1));
tmpCtrl_std     = squeeze(nanstd(out.coh.ctrl,1,1));
...
tmpMask                                 = nan(numFreq,numTime);
tmpMask(logical(inMask.spindleCluster)) = 1;
tmpMask                                 = permute(repmat(tmpMask,[1 1 numRep]),[3 1 2]);

tmpRipple   = out.coh.ripple .* squeeze(tmpMask(1,:,:));
tmpCtrl     = out.coh.ctrl .* tmpMask;

out.stats.spindleCluster.ripple      = mean(tmpRipple,'all','omitnan');
out.stats.spindleCluster.ctrl.mean   = mean(tmpCtrl,'all','omitnan');
out.stats.spindleCluster.ctrl.dist   = mean(tmpCtrl,[2 3],'omitnan');
out.stats.spindleCluster.ctrl.std    = std(mean(tmpCtrl,[2 3],'omitnan'),1,1,'omitnan');
out.stats.spindleCluster.zval        = (out.stats.spindleCluster.ripple - out.stats.spindleCluster.ctrl.mean) / out.stats.spindleCluster.ctrl.std;
out.stats.spindleCluster.pval        = 2*normcdf(-abs(out.stats.spindleCluster.zval));

clear tmpMask tmpRipple tmpCtrl
