...
% ripples and NREM-control events
%
% Requirements:
...
svName      = 'tfr_ripple_nrem';                        %% filename for saved data
svName_mask = 'mask_spindleCluster';                    %% filename for mask with significant spindle cluster (used in follow-up analyses)
...
%% save intersection mask for follow-up analyses
tmp_mask        = squeeze(out.stats.mask(1,:,:)) .* squeeze(out.stats.mask(2,:,:));

outMask       = [];
outMask.time  = out.stats.time;
outMask.freq  = out.stats.freq;
outMask.full  = tmp_mask;

outMask.spindleCluster                             = tmp_mask;
outMask.spindleCluster(1:nearest(inTFR.freq,10),:) = 0;

save(fullfile(dirRoot,[svName_mask '.mat']),'-struct','outSigMask');
